...
import { CarouselForm } from "@/modules/media/components/CarouselForm";
import { carouselService } from "@/modules/media/services/carousel.service";
import { CreateCarouselRequest } from "@/modules/media/types/carousel.types";
...
export default function AddCarousel() {
    ...
    const handleSubmit = async (data: CreateCarouselRequest) => {
        setIsLoading(true);
        try {
            const response = await carouselService.createCarousel(data);
            if (response.success) {
                toast.success(response.message || "Carousel created successfully");
                navigate(ROUTES.MEDIA_CAROUSEL);
            } else {
                toast.error(response.message || "Failed to create carousel");
            }
        } catch (error: any) {
        ...
        <div className="flex min-h-[calc(100vh-120px)] w-full flex-col justify-center py-8 animate-fade-in">
            <div className="mx-auto w-full max-w-2xl space-y-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            Create a new carousel banner slide.
                        </p>
                    </div>
                </div>
                ...
                <div className="rounded-2xl border border-border bg-card p-8 shadow-sm">
                    <CarouselForm onSubmit={handleSubmit} isLoading={isLoading} />
                </div>
            </div>
        </div>
    );
